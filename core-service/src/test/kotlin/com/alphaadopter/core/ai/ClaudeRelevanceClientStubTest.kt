...
// 실제 API를 호출하는 별도 테스트이며 키가 없으면 스킵된다).
class ClaudeRelevanceClientStubTest {
    ...
    private fun clientWithStubbedResponse(body: String): ClaudeRelevanceClient {
        ...
        return ClaudeRelevanceClient(builder, apiKey = "fake-key-for-test", model = "claude-haiku-4-5-20251001")
    }

    @Test
    fun `tool_use 응답에서 score를 정상적으로 파싱한다`() {
        val client = clientWithStubbedResponse(
            body = """{"content":[{"type":"tool_use","name":"score_relevance","input":{"score":87}}]}""",
        )
